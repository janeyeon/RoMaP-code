...
from threestudio.data.multiview import (
...
class SingleImageDataBase:
    def setup(self, cfg, split):
        ...
        if self.cfg.use_random_camera:
            random_camera_cfg = parse_structured(
                MultiviewsDataModuleConfig, self.cfg
            )
            if split == "train":
                self.random_pose_generator = MultiviewIterableDataset(
                    random_camera_cfg
                )
            else:
                self.random_pose_generator = MultiviewDataset(
                    random_camera_cfg, split
                )
