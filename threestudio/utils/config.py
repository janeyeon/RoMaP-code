...
class ExperimentConfig:
    ...
    obj_list: List[int] = field(default_factory=lambda: [4, 7])
    ...
    def __post_init__(self):
        ...
        try:
            self.trial_name = self.tag + "_" + self.system.edit_prompt
        except: 
            self.trial_name = self.tag
            for seg in self.system.seg_list:
                self.trial_name += f"_{seg}"
