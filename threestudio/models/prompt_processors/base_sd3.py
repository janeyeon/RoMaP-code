...
@dataclass
class PromptProcessorOutput_sd3:
    ...
    text_embeddings_pooled: Float[Tensor, "N Nf"]
    uncond_text_embeddings_pooled: Float[Tensor, "N Nf"]
    text_embeddings_vd_pooled: Float[Tensor, "Nv N Nf"]
    uncond_text_embeddings_vd_pooled: Float[Tensor, "Nv N Nf"]
    ...
    def get_text_embeddings(
        ...
        if view_dependent_prompting:
            ...
            text_embeddings_pooled = self.text_embeddings_vd_pooled[direction_idx]  # type: ignore
            uncond_text_embeddings_pooled = self.uncond_text_embeddings_vd_pooled[direction_idx]  # type: ignore
        else:
        ...
        return torch.cat([text_embeddings, uncond_text_embeddings], dim=0),torch.cat([text_embeddings_pooled,uncond_text_embeddings_pooled], dim=0)
...
class PromptProcessor(BaseObject):
    ...
    def load_text_embeddings(self):
        ...
        barrier()
        self.text_embeddings = self.load_from_cache(self.prompt)['embedding'][None, ...]
        
        self.uncond_text_embeddings = self.load_from_cache(self.negative_prompt)['embedding'][
            None, ...
        ]
        self.text_embeddings_pooled = self.load_from_cache(self.prompt)['pooled_embedding'][None, ...]
        self.uncond_text_embeddings_pooled = self.load_from_cache(self.negative_prompt)['pooled_embedding'][
            None, ...
        ]
        ...
        self.text_embeddings_vd_pooled = torch.stack(
            [self.load_from_cache(prompt)['pooled_embedding'] for prompt in self.prompts_vd], dim=0
        )
        self.uncond_text_embeddings_vd_pooled = torch.stack(
            [self.load_from_cache(prompt)['pooled_embedding'] for prompt in self.negative_prompts_vd], dim=0
        )
    ...
    def __call__(self) -> PromptProcessorOutput_sd3:
        return PromptProcessorOutput_sd3(
            ...
            text_embeddings_vd_pooled=self.text_embeddings_vd_pooled,
            uncond_text_embeddings_vd_pooled=self.uncond_text_embeddings_vd_pooled,
