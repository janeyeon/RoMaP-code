...
class StableDiffusionPromptProcessor(PromptProcessor):
    ...
    def spawn_func(pretrained_model_name_or_path, prompts, cache_dir,device):
        ...
        pipe = StableDiffusion3Pipeline.from_pretrained(
                ...
                torch_dtype=torch.float16,
                # text_encoder=None,  # T5 제거
                # tokenizer=None  # T5 관련 tokenizer 제거
                safety_checker=None,feature_extractor= None,requires_safety_checker= False
            )
        pipe = pipe.to(device)
        
        # # tokenizer가 튜플이면 초기화
        # if isinstance(pipe.tokenizer, tuple):
        #     pipe.tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-large-patch14")
        
        # del pipe.text_encoder_3
        # T5 encoder를 포함한 모든 관련 요소 제거
    ...
    # def spawn_func(pretrained_model_name_or_path, prompts, cache_dir,device):
    #     pipe = StableDiffusion3Pipeline.from_pretrained("stabilityai/stable-diffusion-3-medium-diffusers",
    #                                                     torch_dtype=torch.float16,tokenizer=None)
    #     pipe = pipe.to(device)
    #     with torch.no_grad():
    #         (
    #             prompt_embeds,
