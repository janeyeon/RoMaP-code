...
class Camera(nn.Module):
    def __init__(self, c2w, FoVx, FoVy, image, gt_alpha_masks,
                 image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device = "cuda", objects=None, style_transfer=False, height=None, width=None
                 ):
        ...
        if image is not None: 
            ...
            if gt_alpha_masks is not None:
                self.gt_alpha_mask = gt_alpha_masks.to(self.data_device)
                # for gt_alpha_mask in gt_alpha_masks : 
                #     self.masked_images.append(self.original_image * gt_alpha_mask.to(self.data_device))
            else:
